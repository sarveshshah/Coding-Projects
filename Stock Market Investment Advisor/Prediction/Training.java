     ...
     public class thread1 implements Runnable{
       ...
       public void run(){
            ...
            while(t_err > 0.01 )
            {
                ...
                UIStockPrediction.jProgressBar1.setValue(countEpoch/100); //Set value
                UIStockPrediction.jProgressBar1.repaint(); //Refresh graphics
                UIStockPrediction.jProgressBar1.setStringPainted(true);
...
class Training {
   ...
   //    for(int i = 0; i < ntw.hidLayers; i++) {
     ...
     // }
       ...
       // System.out.println("pPrice0="+pPrice[0]);
    }
    
   void readDataPrediction(String fileLocation)
    {
        try {
            BufferedReader br = new BufferedReader(new FileReader(fileLocation));
            String str;
            ...
            while ((str = br.readLine()) != null) {
                String [] stockPrice = str.split(" ");
                input[s++] = (new Double(stockPrice[1]));
                mxn = Math.max(Double.parseDouble(stockPrice[1]),mxn);
            }
            for (int i = 0; i < s; i++ ) {
                input[i] = (input[i]/mxn)*0.8+0.1;
            }
            ...
            predictedPrice = (ntw.testing(input,ntw.hidTemp) - 0.1) * (mxn /0.8);
            System.out.println("\n The predicted price is: " + predictedPrice);
    }

    void DrawStockGraph()
    {
          Prediction pg = new Prediction();
